...
* `Domenico Scarlatti – Keyboard Sonatas <https://github.com/DCMLab/scarlatti_sonatas/archive/refs/heads/main.zip>`__


Data Formats
============

Each piece in this corpus is represented by four files with identical names, each in its own folder. For example, the *Sonata K.1* has the following files:

-  ``MS3/K001.mscx``: Uncompressed MuseScore file including the music and annotation labels.
-  ``notes/K001.tsv``: A table of all note heads contained in the score and their relevant features (not each of them represents an onset, some are tied together)
-  ``measures/K001.tsv``: A table with relevant information about the measures in the score.
-  ``harmonies/K001.tsv``: A list of the included harmony labels (including cadences and phrases) with their positions in the score.
...
.. code:: python
   ...
   labels = ms3.load_tsv('harmonies/K001.tsv')
   notes = ms3.load_tsv('notes/K001.tsv')
...
For questions, remarks etc., please `create an issue <https://github.com/DCMLab/scarlatti_sonatas/issues>`__ and feel free to fork and submit pull requests.
